...
import BillTextBox from '../trading_style/BillTextBox.js'
import DownBox from '../../../common/DownBox.js'
import Pagination from '../../../common/Pagination.js'
import $ from 'jquery'

class BillContentTable extends React.Component {
	constructor(){
		super();
		this.state = {
			showArr: [],
    ...
    render() {
		...
		let rows = this.state.showArr.map((item,i) => (
			<tr>
				...
				<td>{item.xiadan3}</td>
				<td>{item.xiadan4}</td>
				<td style={styles.tableStyle}  data-toggle='modal'><BillTextBox /></td>
			</tr>
		))
		let rowsTitle =  (
			<tr>
				...
				<th >{this.props.dataTitle.details4}</th>
			</tr>
		)
        return (
			<div>
...
BillContentTable.defaultProps={
  ...
  data:[{value: 'serialNumber',name: '全部'}]
}
BillContentTable.propTypes = {
  dataTitle: PropTypes.object.isRequired,
  data: PropTypes.array.isRequired
};

export default BillContentTable;
