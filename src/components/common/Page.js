...
export default class Page extends React.Component{
	...
	componentWillReceiveProps(nextProps){
		if(this.props.size !== nextProps.size){
			this.setState({
				pageIndex:1,
				pageNum: (this.props.total%nextProps.size) == 0 ? this.props.total/nextProps.size : parseInt(this.props.total/nextProps.size + 1)
			})
	...
	pageIndexChange(event){
		...
		if (type == "link") {
			...
			this.state.pageIndexChange(pageIndex-1);
		// }else if (type == "btn-go") {
		// 	let goIndex = this.refs.indexInput;
    ...
    var lastDisplay = this.state.pageNum == this.state.pageIndex ? 'disabled':'';//当前页为最后一页时，尾页和后一页失效
		if(this.state.pageNum <= 5){
			var startIndex = 1 ;
			var endIndex = this.state.pageNum ;
		}else if(this.state.pageIndex <= 3){
      startIndex = 1 ;
      // (Math.ceil(this.state.pageIndex/this.state.showLinkNum)-1) * this.state.showLinkNum + 1;每次显示页数的开始页
      endIndex = 6 ;
      // Math.min(startIndex + this.state.showLinkNum,(this.state.pageNum+1));每次显示页数的结束页
    }else if(this.state.pageIndex >= this.state.pageNum-2){
      startIndex = this.state.pageNum-5;
      endIndex = this.state.pageNum;
    }else{
      startIndex = this.state.pageIndex-2;
      endIndex = parseInt(this.state.pageIndex)+3;
    }
    ...
    arrLast.push(
		...
		return (
			<nav style={{'display':'inline-block'}}  key="page">
            	<ul key='page-ul' className="pagination" onClick = {this.pageIndexChange.bind(this)}>
	              	{arrFirst}
	              	{arrLinkShow}
	                {arrLast}
	               { /*<li>
		               <input type="text" data-type = "input" ref="indexInput" />
		               <a ref= "jump" href="javascript:;" className="btn page-go" data-type = "btn-go">跳转</a>
		            </li>*/}
	           </ul>
       		</nav>
		)
